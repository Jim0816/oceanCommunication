...
export default class index extends Component {
  ...
  message = (msg) => {
    ...
    if (showRange.length == 0){
      ...
      //cur_labels = labels.slice(start, end )
      cur_sub_labels = sub_labels.slice(start, end + 1)
      //cur_sub_labels = sub_labels.slice(start, end )
      ...
      for (let i = 0 ; i < charts.length ; i++){
        ...
        for (let j = 0 ; j < chart.values.length ; j++) {
          ...
          //values_in_new_chart.push({"name": chart.values[j].name, "value": chart.values[j].value.slice(start, end )})
        }
        let new_chart = {
          "title": chart.title,
